...
1. 多粒度增量爬取
   * 将数据库中论文从粗到细划分为多个区间，记录每个区间爬取结果，避免重复爬取。
   * 保证发生故障后高效断点续爬。
2. 自动防反爬
   * 根据真实数据库中的使用统计信息，自动获取对应用户代理防反爬。
   * 增加爬虫的鲁棒性。
3. 相关信息爬取对齐
   * 根据论文数据在视频、PPT网站中进行检索爬取，有效保证视频与论文的相关性。
4. 检索排序
   * 根据论文与查询字符串的匹配程度、发表年份、引用数进行排序，保证查询结果的有效性。
5. 分页查询
...
爬虫环境主要依赖MongoDB数据库与Scrapy爬虫框架，适用于MacOS，Windows，Linux等多种操作系统。这里以Ubuntu18.04为例，介绍具体配置方法。
...
### 1 爬虫模块
   ...
   - 为了获取论文与电子书相关信息，本项目重点爬取了 ACM DL 及 library genesis 两个网站，并通过 SciHub 、 slideserve 、Youtube 等网站获取相关的pdf、ppt及视频等信息。
   - 以 ACM DL为例，其爬取的工作流程主要分为收集论文/电子书列表，爬取论文/电子书主要信息，爬取对应PPT 和视频三个部分，具体工作流程如下图所示：

      <img src="https://s2.loli.net/2022/12/24/L4VFOhzMvdDNbTn.png" alt="img" style="zoom: 33%;" />
...
#### 1.1 设计原理
   ...
   - 收集论文 / 电子书列表 ：
     - 以5天为1段，构造论文 / 电子书 数据库初始列表页URL
     - 爬取对应列表页，解析列表中项目（论文 / 电子书）详情页URL
     - 爬取当前页所列项目主要信息（见下）
     - 当前项目列表页论文爬取完毕后，构造下一列表页URL，继续爬取获得项目列表，直至当前列表页无项目

   - 爬取论文 / 电子书主要信息：
     - 依据解析所得项目详情页URL，爬取项目信息
     - 提取论文基本信息
     - 判断对应pdf是否可直接下载，若可，直接保存对应pdf的URL；若不可，依据其doi编号，通过scihub获得pdf的URL
     - 将数据保存入MongoDB

   - 爬取对应 PPT 和视频 ：
     - 读取已爬取的论文/电子书数据，将标题作为检索query，在PPT Silver、Youtube等网站爬取对应内容
     - 解析返回结果，获取视频或PPT标题与链接
     - 筛选排序名次靠前的视频与PPT，将对应url写入MongoDB
...
- 覆盖率如下：
  ...
  | venue         | 54207 | 100.00% |
  | volume        | 54207 | 100.00% |
  | citations_num | 54207 | 100.00% |
  | source_url    | 54207 | 100.00% |
  | pdf_url       | 41153 | 75.92%  |
  | ppt_url       | 3997  | 7.37%   |
  | video_url     | 22948 | 42.33%  |
